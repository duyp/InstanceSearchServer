function time = query_groundtruth_tfidf_myinv(dict_words, inv_file, dict_structure, groundtruthDir, datasetDir)
...
%returns: after ranking, this function will run script to compute mAP from result directory
    ...
    for k = 1:length(q_files)
        ...
        tic;
        [ranked_ids, ~] = inv_search_c(inv_file,q_words{1},'l1','nhi');
        time(k) = toc;
        fprintf('size: %d\n',length(ranked_ids));
        ...
        script = [script_file, ...
            ...
            ' >result\oxford5k\new-propose-tfidf-nhi\tfidf-nhi-l1norm\', image_name(6:end), '_result.txt']; %q_files(k).name(1:end-10)
        system(script);
        if verbose==1
            pause;
        end
